...
#define TC0_IS_TRUE(mode,OC0,clk,INT)	( TC0_TRUE_OC0(mode,OC0)&&TC0_TRUE_CLK(clk)&&TC0_TRUE_INT(INT) )
...
/* CLOCK MODE:
** the Pre-scaler(6 > CSn2:0 > 1) shared betwwen timer0 and timer1, when it set its value you actually set the two timers prescaller 
** Also when you reset the pre-scaler from SFIOR at PSR10 bit, you actually reset the two timers pre-scaler.
*/
...
#define TC0_TRUE_CLK(clk)			(clk>=TC0_DISABLE_CLK&&clk<=TC0_EXTCLK_T0T1PINS_RISEE)
...
#define TC0_StopTimer0()				TCCR0 |= TC0_DISABLE_CLK
#define TC0_READ_TC0VALUE				TCNT0&0xFF
