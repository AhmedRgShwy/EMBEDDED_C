...
#define TC1_TRUE_PWMTOGGLEOC1A_MODE(mode)		(mode==TC1_FASTPWM_OCR1ATOP||mode==TC1_FASTPWM_ICR1TOP||mode==TC1_PWMPHASEFREQENCY_OCR1ATOP)
#define TC1_NOT_PWMTOGGLEOC1A_MODE(mode)		(mode!=TC1_FASTPWM_OCR1ATOP||mode!=TC1_FASTPWM_ICR1TOP||mode!=TC1_PWMPHASEFREQENCY_OCR1ATOP)
...
/* CLOCK MODE:
** the prescaler(6 > CSn2:0 > 1) shared betwwen timer1 and timer0, when it set its value you actually set the two timers prescaler.
...
#define TC1_TC0_CLKPRESCALE_8		0x02
#define TC1_TC0_CLKPRESCALE_64		0x03
#define TC1_TC0_CLKPRESCALE_256		0x04
#define TC1_TC0_CLKPRESCALE_1024	0x05
#define TC1_EXTCLK_T0T1PINS_FALLE 	0x06
#define TC1_EXTCLK_T0T1PINS_RISEE	0x07
#define TC1_TRUE_CLK(clk)			(clk>=TC1_DISABLE_CLK && clk<=TC1_EXTCLK_T0T1PINS_RISEE)

#define TC1_TC0_Prescaler_Reset()	SET_BIT(SFIOR,PSR10)
...
#define TC1_AC0_NO_NFILTER_FEDGE			0x10
#define TC1_AC0_NO_NEFILTER_REDGE			0x11
#define TC1_AC0_NFILTER_FEDGE				0x12
#define TC1_AC0_NFILTER_REDGE				0x13
#define TC1_TRUE_CAP_PINSRC_ADJUST(adjust)	(	adjust>=TC1_ICP1_NO_NFILTER_FEDGE && adjust<=TC1_ICP1_NFILTER_REDGE&&\
												adjust>=TC1_AC0_NO_NFILTER_FEDGE && adjust<=TC1_AC0_NFILTER_REDGE	)

#define TC1_AlterTo_FEdge()		CLR_BIT(TCCR1B,ICES1)
#define TC1_AlterTo_REdge()		SET_BIT(TCCR1B,ICES1)
...
/* CLEAR ITERRUPTS FLAGS ( didn't committed in GitHub ) */

/* clearing this flag when you are measuring a duty cycle for a pulse using capture ICP1 and altering ICP1 edge is needed,
** is must between altering  */
#define TC1_CLR_INT_Capture_Flag()		CLR_BIT(TIFR,ICF1)
#define TC1_CLR_INT_CMP1A_Flag()		CLR_BIT(TIFR,OCF1A)
#define TC1_CLR_INT_CMP1B_Flag()		CLR_BIT(TIFR,OCF1B)
#define TC1_CLR_INT_OVF_Flag()			CLR_BIT(TIFR,TOV1)
#define TC1_StopTimer1()				TCCR1B |= TC1_DISABLE_CLK



/* READ CAPTURE REGISTER AFTER YOU HAVE TRIGGERED ICP1 */
#define TC1_READ_CAPTURE				ICR1&0xFFFF
#define TC1_READ_TC1VALUE				TCNT1&0xFFFF
...
#define TC1_IS_TRUE(mode,clk,INT,OC1A,OC1B,ICP1)		TC1_TRUE_CLK(clk)&&TC1_TRUE_INTMODE(INT)&&\
														(TC1_TRUE_OC1x(mode,OC1A)&&TC1_TRUE_OC1x(mode,OC1B))&&\
														TC1_TRUE_CAP_PINSRC_ADJUST(ICP1)
...
/************************************************************************************************************************
** => FUNCTION FEATURES:
** This function initials TIMER1 with desired specifications it returns true when it succeeded, otherwise returns false.
